...
def create_interaction_matrices(
    parameter_file, NewMODELconfig={}, NewPWconfig={}, NewEFCconfig={}
):
    ...
    config = ConfigObj(
        parameter_file, configspec=configspec_file, default_encoding="utf8"
    )
    vtor = Validator()
    ...
    if not os.path.exists(configspec_file):
        raise Exception(
            "The parameter config file " + configspec_file + " cannot be found"
        )
    ...
    intermatrix_dir = (
        ...
        + coronagraph
        + "/"
        + str(int(wavelength * 1e9))
        + "nm/p"
        + str(round(pdiam * 1e3, 2))
        + "_l"
        + str(round(lyotdiam * 1e3, 1))
        + "/ldp_"
        + str(round(ld_p / lyot, 2))
        + "/basis_"
        + basistr
        + "/"
    )
    ...
    for i in np.arange(len(posprobes)):
        ...
        vectorPW[
            0, i * dimimages * dimimages : (i + 1) * dimimages * dimimages
        ] = vectoressai[:, 0, i].flatten()
        vectorPW[
            1, i * dimimages * dimimages : (i + 1) * dimimages * dimimages
        ] = vectoressai[:, 1, i].flatten()
    ...
    print("TO SET ON LABVIEW: ", str(150 / 2 + np.array(np.fft.fftshift(choosepix))))
    # Creating WhichInPup?
    ...
    fits.writeto(
        Labview_dir + "Matrix_control_EFC_DM3_default.fits", EFCmatrix, overwrite=True
    )
...
def correctionLoop(
    parameter_file, NewMODELconfig={}, NewPWconfig={}, NewEFCconfig={}, NewSIMUconfig={}
):
    ...
    if correction_algorithm == "EM" or correction_algorithm == "steepest":
        ...
        G = (
            Gmatrix[0 : int(Gmatrix.shape[0] / 2), :]
    ...
    if set_amplitude_abb == True:
        ...
        ampfinal[
            int(isz / 2 - len(amp1) / 2) + 1 : int(isz / 2 + len(amp1) / 2) + 1,
            int(isz / 2 - len(amp1) / 2) + 1 : int(isz / 2 + len(amp1) / 2) + 1,
        ] = amp1
        ampfinal = (ampfinal) * instr.roundpupil(isz, prad - 1)
        moy = np.mean(ampfinal[np.where(ampfinal != 0)])
        ampfinal = (ampfinal / moy - np.ones((isz, isz))) * instr.roundpupil(
            isz, prad - 1
        )  # /10
    else:
        ampfinal = 0
    ...
    maskDHisz = wsc.creatingMaskDH(
        isz, "square", choosepixDH=[element * isz / dimimages for element in choosepix]
    )
    ...
    for mode in modevector:
        ...
        if correction_algorithm == "EFC":
            ...
            solution1 = wsc.solutionEFC(
                maskDH, resultatestimation, invertGDH, WhichInPupil
            )
        ...
        if correction_algorithm == "EM":
            ...
            solution1 = wsc.solutionEM(
                maskDH, resultatestimation, invertM0, G, WhichInPupil
            )

        if correction_algorithm == "steepest":
            solution1 = wsc.solutionSteepest(
                maskDH, resultatestimation, M0, G, WhichInPupil
            )

        apply_on_DM = (
            -gain
            * amplitudeEFC
            * np.dot(solution1, pushactonDM.reshape(1024, isz * isz)).reshape(isz, isz)
            * 2
            * np.pi
            * 1e-9
            / wavelength
        ...
        if photon_noise == True:
            photondetector[k + 1] = np.random.poisson(
                imagedetector[k + 1] * contrast_to_photons
            )
