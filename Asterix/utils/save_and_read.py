    ...
    if isinstance(config, ConfigObj):
        for sect in config.sections:
            for scalar in config[str(sect)].scalars:
                header.set(str(scalar).upper(), str(config[str(sect)][str(scalar)]))
    elif isinstance(config, Section):
        for scalar in config.scalars:
            header.set(str(scalar).upper(), str(config[str(scalar)]))
    elif isinstance(config, dict):
        for key in config.keys():
            header.set(str(key).upper(), str(config[str(key)]))
    else:
