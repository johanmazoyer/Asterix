...
def contrast_curves(reduced_data, xcen=None, ycen=None, delta_raddii=3, type_of_contrast='mean', mask_DH=None):
    ...
    type_of_contrast : string, default 'mean'
        Can be  'mean' : mean contrast on the rings.
                'stddev_1sig' : 1 sigma standard deviation on the rings.
                'stddev_5sig' : 5 sigmas standard deviation on the rings.
    mask_DH : 2d binary Array, default is all focal plane
        Mask delimiting the DH.
...
def plot_err_probe_curve(Err_estim, config, probe_dir):
    """create a contrast curve to plot the probe error between perfect and estimated field
